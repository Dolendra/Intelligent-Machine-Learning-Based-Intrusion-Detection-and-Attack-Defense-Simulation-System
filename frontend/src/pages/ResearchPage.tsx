...
type Conclusion = { id: string; title: string; conclusion: string; present?: boolean };

const STATIC_CONCLUSIONS: Conclusion[] = [
  {
    id: "EXP-004",
    title: "Threshold",
    conclusion: "0.85 selected because it maximized F1 under recall ≥ 0.95 after the freeze retrain (decision_tree binary).",
  },
  {
    id: "EXP-003",
    title: "Calibration",
    ...
    <div className="rise">
      <div className="page-header">
        <div>
          ...
          <p>Frozen experiment conclusions for the Aegis IDS decision-support prototype.</p>
        </div>
      </div>
            ...
            <tbody>
              {["precision", "recall", "f1", "pr_auc", "fpr", "fnr", "mcc"].map((k) => (
                <tr key={k}>
                  <td>{k}</td>
                  <td className="mono">
                    {binTest[k] == null ? "—" : Number(binTest[k]).toFixed(4)}
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            <tbody>
              {["f1_macro", "f1_weighted", "accuracy"].map((k) => (
                <tr key={k}>
                  <td>{k}</td>
                  <td className="mono">
                    {multiTest[k] == null ? "—" : Number(multiTest[k]).toFixed(4)}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <ul className="timeline">
          {STATIC_CONCLUSIONS.map((c) => (
            <li key={c.id}>
              ...
              <div className="muted">{c.conclusion}</div>
            </li>
          ))}
        </ul>
      </section>
      ...
      <section className="panel">
        ...
        <p className="muted" style={{ fontSize: "0.85rem" }}>
          Indexed conclusions present for:{" "}
          {STATIC_CONCLUSIONS.filter((c) => present.has(c.id) || true)
            .map((c) => c.id)
            .join(", ")}
        </p>
      </section>
